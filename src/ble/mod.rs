...
impl CentralManager {
    ...
    fn handle_connected(self: &Self, sender: Sender<EventMessage>, bd_addr: BDAddr) -> Result<()> {
        ...
        match peripheral {
            Some(peripheral) => {
                log::debug!("Connected {:?} with {:?}", peripheral.properties().local_name, peripheral.properties());

                let value_peripheral_clone = peripheral.clone();
                let value_sender = sender.clone();
                let value_char_uuid: UUID = "85:6D:27:BF:B8:E1:41:09:BF:61:57:33:F4:E1:B2:99".parse()?;
                peripheral.on_discovery(value_char_uuid, Box::new(move |discovery| {
                    ...
                    match result {
                        Ok(_) => {}
                        Err(err) => {
                            log::error!("Failed to do something with characteristic discovery: {:?}", err);
                        }
                ...
                let data_char_uuid: UUID = "97:87:A5:54:76:CC:4D:02:99:BB:AA:7D:5A:4F:4A:99".parse()?;
                ...
                task::block_on(async {
                    sender.send(EventMessage::PendingDisconnect(
    ...
    pub fn run(self) -> Result<()> {
        ...
        task::spawn(async move {
            ...
            loop {
                ...
                let event_message: EventMessage = task::block_on(async {
...
mod tests {
    ...
    fn parse_asset_uuids() {
        ...
        for input in [value_char_uuid, data_char_uuid, version_char_uuid, error_char_uuid].iter() {
